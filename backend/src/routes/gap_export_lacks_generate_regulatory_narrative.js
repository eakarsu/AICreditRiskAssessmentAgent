/**
 * Gap Feature: gap-export-lacks-generate-regulatory-narrative
 * 'export' lacks '/generate-regulatory-narrative'.
 *
 * POST /api/gap-export-lacks-generate-regulatory-narrative
 * Auth required. Generated as part of Gaps backend scaffold (batch_02).
 * Integration credentials: process.env.FEATURE_GAP_EXPORT_LACKS_GENERATE_REGULATORY_NARRATIVE_KEY
...
const FEATURE_KEY = process.env.FEATURE_GAP_EXPORT_LACKS_GENERATE_REGULATORY_NARRATIVE_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: 'export' lacks '/generate-regulatory-narrative'..\nGoal: 'export' lacks '/generate-regulatory-narrative'.\nRespond with clear, actionable analysis. Prefer JSON when structured output is requested.`;
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AICreditRiskAssessmentAgent - gap-export-lacks-generate-regulatory-narrative',
    },
    body: JSON.stringify({
...
router.post('/', auth, async (req, res) => {
  try {
    ...
    try {
      ...
      if (pool && pool.query) {
        await pool.query(
          ...
          ['gap-export-lacks-generate-regulatory-narrative', JSON.stringify(payload), JSON.stringify(ai.result), ai.model, req.user?.id || null]
        );
      }
    } catch (persistErr) {
      console.warn('[gap-export-lacks-generate-regulatory-narrative] persistence skipped:', persistErr.message);
    }
    return res.json({
      ok: true,
      feature: 'gap-export-lacks-generate-regulatory-narrative',
      endpoint: '/api/gap-export-lacks-generate-regulatory-narrative',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
    ...
    console.error('[gap-export-lacks-generate-regulatory-narrative] error:', err.message);

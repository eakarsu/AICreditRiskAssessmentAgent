/**
 * Custom Feature: concentration
 * Portfolio concentration analysis
 *
 * POST /api/ai/concentration
 * Auth required. Generated as part of Custom Feature Suggestions scaffold (batch_02).
 * Integration credentials: process.env.FEATURE_CONCENTRATION_KEY
...
const FEATURE_KEY = process.env.FEATURE_CONCENTRATION_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: Portfolio concentration analysis.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AICreditRiskAssessmentAgent - concentration',
    },
    body: JSON.stringify({
...
router.post('/concentration', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    if (!payload || Object.keys(payload).length === 0) {
      return res.status(400).json({ error: 'Request body is required' });
    }
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_CONCENTRATION_KEY');
    const ai = await callLLM({ feature: 'concentration', goal: 'Portfolio concentration analysis', input: payload });
    // No persistence wired.
    return res.json({
      ok: true,
      feature: 'concentration',
      endpoint: '/api/ai/concentration',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
...
router.get('/concentration/health', (req, res) => {
  res.json({
    feature: 'concentration',
    endpoint: '/api/ai/concentration',
